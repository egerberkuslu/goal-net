import {
  WHISTLE_DELAY_MS, WHISTLE_EVENTS, createVoiceState, pickVoice,
} from './voiceLines.js';

const VOL_KEY = 'goalnet-vol';

// The commentator's clips, recorded by tools/make-voice.mjs into ./voice/.
//
// Discovered at build time with import.meta.glob so they ride through `vite
// build` as fingerprinted assets — dist-assets/ is gitignored and is not
// copied by a build, which is why nothing shipped may live there. Under node
// import.meta.glob is undefined and the call throws into the catch: an empty
// map, and every say() returns on its first line. A checkout that has not run
// `npm run gen:voice` behaves the same way: silent, never broken.
const VOICE_URLS = (() => {
  try {
    const found = import.meta.glob('./voice/*.opus', {
      eager: true, query: '?url', import: 'default',
    });
    const map = Object.create(null);
    for (const [path, url] of Object.entries(found)) {
      map[path.slice(path.lastIndexOf('/') + 1, -'.opus'.length)] = url;
    }
    return map;
  } catch {
    return {};
...
// The menu track, generated by tools/make-music.mjs into ./music/. Same
// discovery, same fallback: no file, no music, no error.
const MUSIC_URL = (() => {
  try {
    const found = import.meta.glob('./music/*.opus', {
      eager: true, query: '?url', import: 'default',
    });
...
export class Sfx {
  constructor() {
    ...
    this.ambTarget = 0;
    this.bed = null;
    this.voiceState = createVoiceState();
    this.voiceBuf = new Map();     // url -> Promise<AudioBuffer|null>
    this.voiceSrc = null;          // the line in flight, if any
    this.voiceSrcPriority = 0;
    this.musicTarget = 0;
    this.music = null;             // gain node, built with the context
    this.musicSrc = null;          // the looping source, started once
    this.musicBuf = null;          // Promise<AudioBuffer|null>
    ...
    if (!this.ctx) {
      ...
      // The commentator sits under the same master as everything else, so the
      // one volume slider the game has covers him too.
      this.voice = this.ctx.createGain();
      this.voice.gain.value = VOICE_GAIN;
      this.voice.connect(this.master);
      // Decode every line now (about 80 KB in all) so the first goal of the
      // match is not also the first fetch.
      for (const url of Object.values(VOICE_URLS)) this.#loadVoice(url);
  ...
  #loadVoice(url) {
    if (this.voiceBuf.has(url)) return this.voiceBuf.get(url);
    if (typeof fetch !== 'function') return Promise.resolve(null);
    const p = fetch(url)
      .then((r) => r.arrayBuffer())
      .then((b) => this.ctx.decodeAudioData(b))
      // A clip that will not fetch or decode is silence for the rest of the
      // session, not an error every time it is asked for.
      .catch(() => null);
    this.voiceBuf.set(url, p);
    return p;
  }

  /**
   * Say one line, by id from view/voiceLines.js.
   *
   * A line already in flight is only cut off by a goal; anything else that
   * arrives while he is talking is dropped rather than queued, because a
   * commentator who catches up on the last four seconds is worse than one who
   * missed a throw-in.
   *
   * @param {string} id
   * @param {{delayMs?:number, priority?:number}} [opts]
   */
  say(id, { delayMs = 0, priority = 0 } = {}) {
    const url = VOICE_URLS[id];
    if (!url || !this.ensure() || this.volume <= 0) return;
    this.#loadVoice(url).then((buf) => {
      if (!buf || !this.ctx) return;
      if (this.voiceSrc) {
        if (priority < 2) return;
        try { this.voiceSrc.stop(); } catch { /* already ended */ }
      }
      ...
      src.start(at);
      this.voiceSrc = src;
      this.voiceSrcPriority = priority;
      this.voiceState.busyUntil = performance.now() + delayMs + buf.duration * 1000;
      src.onended = () => { if (this.voiceSrc === src) this.voiceSrc = null; };
    });
  }

  /**
   * React to a world event with a line, subject to the anti-spam rules in
   * voiceLines.js. On a whistle event the referee's synth whistle goes first
   * and the commentator follows a beat later.
   */
  comment(eventType) {
    const id = pickVoice(this.voiceState, eventType, performance.now());
    if (!id) return;
    const priority = eventType === 'goal' || eventType === 'golden' ? 2 : 0;
    this.say(id, {
      delayMs: WHISTLE_EVENTS.has(eventType) ? WHISTLE_DELAY_MS : 0, priority,
    });
  }

  /**
   * Menu music, 0..1. Idempotent like setAmbiance, so it is called every
   * frame. The track is fetched and decoded the first time it is asked for
   * above zero, looped, and only ever faded — never restarted — so leaving
   * the menu and coming back resumes the same bar rather than the intro.
   */
  setMusic(level) {
    const v = Math.max(0, Math.min(1, Number.isFinite(level) ? level : 0));
    if (Math.abs(v - this.musicTarget) < 0.01) return;
    this.musicTarget = v;
    if (!MUSIC_URL || !this.ensure()) return;
    if (!this.musicBuf) {
      this.musicBuf = (typeof fetch === 'function'
        ? fetch(MUSIC_URL).then((r) => r.arrayBuffer()).then((b) => this.ctx.decodeAudioData(b))
        : Promise.resolve(null)
      ).catch(() => null);
      this.musicBuf.then((buf) => {
        if (!buf || !this.ctx || this.musicSrc) return;
        const src = this.ctx.createBufferSource();
        src.buffer = buf;
        src.loop = true;
        ...
        src.start();
        this.musicSrc = src;
        this.music.gain.setTargetAtTime(MUSIC_GAIN * this.musicTarget, this.ctx.currentTime, MUSIC_RAMP);
      });
      return;
    }
    if (this.musicSrc) {
      this.music.gain.setTargetAtTime(MUSIC_GAIN * v, this.ctx.currentTime, MUSIC_RAMP);
    }
  }
